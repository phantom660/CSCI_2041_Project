(* These are the functions and modules I get from the module in the lib directory.
   I've called it Project.ml there, some will probably have called it halloween.ml.
   ...
   let string_of_declaration = Project.string_of_expression
   let mainParser = Project.Parser.main
   let mainLexer = Project.Lexer.token
   module Parser : (sig exception Error end) = Project.Parser
   (* Here are the types these things have for me,
      ...
      Parser.main : (Lexing.lexbuf -> token) -> Lexing.lexbuf -> Project.expression list
      Lexer.token : Lexing.lexbuf -> Project.Parser.token
      string_of_declaration : Project.expression -> string
   *)
